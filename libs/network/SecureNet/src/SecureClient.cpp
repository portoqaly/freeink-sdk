...
int SecureClient::connectWithMethod(const char* host, uint16_t port, void* method, const char* label) {
  ...
  if (_insecure) {
    ...
    const int loadResult = wolfSSL_CTX_load_verify_buffer(ctx, reinterpret_cast<const unsigned char*>(_rootCA),
    ...
    if (Serial) Serial.printf("[SecureClient] no trust policy configured (%s)\n", label);
    stop();
    return 0;
  }
  ...
  if (!_insecure && wolfSSL_check_domain_name(ssl, host) != WOLFSSL_SUCCESS) {
    if (Serial) Serial.printf("[SecureClient] hostname check setup failed (%s)\n", label);
    stop();
    return 0;
  }
