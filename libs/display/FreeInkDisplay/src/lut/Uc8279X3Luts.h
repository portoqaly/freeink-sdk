...
// --- B/W DU TURBO : 5 x 43, command-prefixed. PENDING HARDWARE VALIDATION. ---
// Derived from BW_DU by halving the long drive phases (x7 -> x3, x6 -> x2)
// while keeping every 1-frame structure byte and all polarity high bits.
// Roughly 15 drive frames against the stock ~31, so the waveform should land
// near half the measured 457 ms fast cycle. Weaker development: lighter
// blacks and faster ghost accumulation are expected — qualify with the
// x3bench refresh laboratory before promoting beyond Flash playback.
inline const uint8_t kUc8279X3_BwDuTurbo[5][43] = {
    {0x20, 0x01, 0x03, 0x01, 0x02, 0x02, 0x01, 0x01, 0x01, 0x02, 0x01, 0x00, 0x00, 0x01},
    {0x21, 0x01, 0x03, 0x81, 0x02, 0x02, 0x01, 0x01, 0x01, 0x02, 0x01, 0x00, 0x00, 0x01},
...
// --- B/W DU ULTRA : 5 x 43, command-prefixed. BENCH-ONLY CANDIDATE. ---
// One aggressiveness step further (x7 -> x2, x6 -> x1): ~11 drive frames,
// aimed at the 350 WPM single-word target (~171 ms cycle). Expect visible
// contrast loss; measure timing and photograph ghosting in x3bench before
// any reader use.
inline const uint8_t kUc8279X3_BwDuUltra[5][43] = {
    {0x20, 0x01, 0x02, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x01},
    {0x21, 0x01, 0x02, 0x81, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x01},
